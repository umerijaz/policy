# Policy Objective: Municipal grants to install smart overflow sensors in sewer catchments to prevent toxic spills.
...
# program_cost represents the cost of installing the smart sensor grid per catchment zone ($45k)
program_cost    <- 45000    
budget_capacity <- 0.25    # The municipality only has budget to retrofit 25% of the network

# Target Key Mapping
outcome_var   <- "Metric_Y"       
treatment_var <- "Action_D"       
covariates    <- c("Feature_SewerType", "Feature_Catchment", "Feature_AssetAge", "Feature_PriorSpills") 

cat("📡 Synthesizing municipal wastewater network dataset directly in-memory...\n")

# Initialize Sample Space (2,500 distinct municipal catchment zones)
N_samples <- 2500

# 1. Infrastructure Characteristics
Feature_SewerType <- sample(c("Combined Sewer (CSS)", "Separate Sewer (SSS)"), N_samples, replace = TRUE, prob = c(0.35, 0.65))
Feature_Catchment <- sample(c("Urban Core", "Suburban", "Industrial"), N_samples, replace = TRUE, prob = c(0.30, 0.50, 0.20))
Feature_AssetAge  <- round(runif(N_samples, 15, 95)) # Age of the underground pipes in years

# 2. Prior Baseline Metrics (CSO Spill Events per Year)
# Combined sewers inherently spill more often, especially as they age
lambda_spills <- ifelse(Feature_SewerType == "Combined Sewer (CSS)", 8, 1) + (Feature_AssetAge * 0.05)
Feature_PriorSpills <- rpois(N_samples, lambda = lambda_spills)

# 3. Treatment Assignment (Action_D: Received the Smart Sensor Retrofit Grant)
# Selection Bias: Zones with higher prior spills and older assets lobbied harder for the grants
propensity <- numeric(N_samples)
for (i in 1:N_samples) {
  logit <- -1.5 + (0.15 * Feature_PriorSpills[i]) + (0.01 * Feature_AssetAge[i])
  p <- 1 / (1 + exp(-logit))
  propensity[i] <- max(0.15, min(0.85, p)) # Ensure overlap
}
Action_D <- rbinom(N_samples, 1, propensity)

# 4. Outcome Construction (Metric_Y = Avoided Damages, Clean-up Costs, and Fines in $)
# Baseline natural efficiency savings without sensors
baseline_Y <- 2000 + (50 * Feature_AssetAge) + rnorm(N_samples, mean = 0, sd = 1500)

# 5. Inject Heterogeneous Treatment Effects (True CATE)
# Smart sensors prevent massive fines mostly in old, urban combined sewers. 
# In suburban separate sewers, they just provide minor operational maintenance savings.
true_effect <- numeric(N_samples)
for (i in 1:N_samples) {
  eff <- 5000 # Base benefit of better data
  
  if (Feature_SewerType[i] == "Combined Sewer (CSS)") {
    eff <- eff + 40000 
    if (Feature_Catchment[i] == "Urban Core") {
      eff <- eff + 35000 # High density means higher avoided flooding damages
    }
  } else {
    if (Feature_Catchment[i] == "Industrial") {
      eff <- eff + 15000 # Catching industrial illicit discharges
    }
  }
  
  # Age multiplier: Sensors are much more valuable on older infrastructure prone to failure
  eff <- eff + (Feature_AssetAge[i] * 350)
  true_effect[i] <- eff
}

# Final Financial Outcome with residual noise
Metric_Y <- round(baseline_Y + (Action_D * true_effect) + rnorm(N_samples, mean = 0, sd = 8000))
Metric_Y <- pmax(0, Metric_Y) # Cannot have negative avoided costs

raw_data <- data.frame(
  Metric_Y, 
  Action_D, 
  Feature_SewerType = as.factor(Feature_SewerType), 
  Feature_Catchment = as.factor(Feature_Catchment), 
  Feature_AssetAge, 
  Feature_PriorSpills
)

cat(sprintf("Cohort Synthesis Complete: %d infrastructure zones loaded.\n", nrow(raw_data)))
