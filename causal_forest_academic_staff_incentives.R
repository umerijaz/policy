#Policy Objective: Evaluating the causal return of providing financial seed capital to academic researchers.
...
# program_cost represents the "break-even" threshold in publication impact factor units
program_cost    <- 5.0    
budget_capacity <- 0.25    # Funding capacity (e.g., granting seed money to 25% of applicants)

# Target Key Mapping
outcome_var   <- "Metric_Y"       
treatment_var <- "Action_D"       
covariates    <- c("Feature_Rank", "Feature_Field", "Feature_Tier", "Feature_YearsPhD", "Feature_PriorImpact") 

cat("📡 Synthesizing academic pilot study cohort dataset directly in-memory...\n")

# Initialize Sample Space
N_samples <- 2000

# 1. Academic Rank Distribution
ranks      <- c("Postdoc", "Assistant Professor", "Associate Professor", "Full Professor")
rank_probs <- c(0.15, 0.35, 0.30, 0.20)
Feature_Rank <- sample(ranks, N_samples, replace = TRUE, prob = rank_probs)

# 2. Research Field Distribution
fields      <- c("STEM", "Social Sciences", "Humanities")
field_probs <- c(0.45, 0.35, 0.20)
Feature_Field <- sample(fields, N_samples, replace = TRUE, prob = field_probs)

# 3. Institution Tier Distribution
tiers      <- c("Tier_1", "Tier_2", "Tier_3")
tier_probs <- c(0.30, 0.50, 0.20)
Feature_Tier <- sample(tiers, N_samples, replace = TRUE, prob = tier_probs)

# 4. Bounded Years Since PhD (Conditional on Academic Rank)
Feature_YearsPhD <- numeric(N_samples)
for (i in 1:N_samples) {
  if (Feature_Rank[i] == "Postdoc") {
    Feature_YearsPhD[i] <- sample(1:4, 1)
  } else if (Feature_Rank[i] == "Assistant Professor") {
    Feature_YearsPhD[i] <- sample(1:7, 1)
  } else if (Feature_Rank[i] == "Associate Professor") {
    Feature_YearsPhD[i] <- sample(6:15, 1)
  } else { # Full Professor
    Feature_YearsPhD[i] <- sample(12:35, 1)
  }
}

# 5. Prior Impact Score (Baseline Productivity before the study)
# Accounts for structurally higher citation scales in STEM vs. Humanities
base_impact     <- c("STEM" = 15, "Social Sciences" = 8, "Humanities" = 3)
rank_multiplier <- c("Postdoc" = 0.5, "Assistant Professor" = 1.0, "Associate Professor" = 1.5, "Full Professor" = 2.2)
tier_multiplier <- c("Tier_1" = 1.4, "Tier_2" = 1.0, "Tier_3" = 0.7)

Feature_PriorImpact <- numeric(N_samples)
for (i in 1:N_samples) {
  val <- base_impact[Feature_Field[i]] * rank_multiplier[Feature_Rank[i]] * tier_multiplier[Feature_Tier[i]]
  Feature_PriorImpact[i] <- max(0.1, val + rnorm(1, mean = 0, sd = val * 0.25))
}

# 6. Treatment Assignment (Action_D) with Logistic Propensity (Simulating Real Selection Bias)
# Higher tier institutions and higher baseline performers have a slightly higher chance of grant capture
propensity <- numeric(N_samples)
for (i in 1:N_samples) {
  logit <- -0.2 + 0.3 * (Feature_Tier[i] == "Tier_1") - 0.2 * (Feature_Tier[i] == "Tier_3") + 0.02 * Feature_PriorImpact[i]
  p <- 1 / (1 + exp(-logit))
  propensity[i] <- max(0.2, min(0.8, p)) # Enforces strong common support bounds
}
Action_D <- rbinom(N_samples, 1, propensity)

# 7. Outcome Construction (Metric_Y = Post-incentive Publications * Impact Factor)
baseline_Y <- numeric(N_samples)
for (i in 1:N_samples) {
  b_val <- Feature_PriorImpact[i] * 0.8 + 0.5 * Feature_YearsPhD[i]
  if (Feature_Field[i] == "STEM")            b_val <- b_val + 5
  if (Feature_Field[i] == "Social Sciences") b_val <- b_val + 2
  baseline_Y[i] <- b_val
}

# 8. Inject Nuanced Heterogeneous Treatment Effects (True CATE)
# Early-mid career scholars in capital-intensive fields yield the highest marginal return per dollar
true_effect <- numeric(N_samples)
for (i in 1:N_samples) {
  eff <- 0.0
  if (Feature_Field[i] == "STEM") {
    if (Feature_Rank[i] == "Assistant Professor")      eff <- 12.0
    else if (Feature_Rank[i] == "Associate Professor") eff <- 8.0
    else if (Feature_Rank[i] == "Postdoc")             eff <- 5.0
    else                                               eff <- 2.0 # Saturated Full Profs
  } else if (Feature_Field[i] == "Social Sciences") {
    if (Feature_Rank[i] == "Assistant Professor")      eff <- 7.0
    else if (Feature_Rank[i] == "Associate Professor") eff <- 5.0
    else if (Feature_Rank[i] == "Postdoc")             eff <- 3.0
    else                                               eff <- 1.0
  } else { # Humanities (uniform time buyout/archives access lift)
    eff <- if (Feature_Rank[i] %in% c("Assistant Professor", "Associate Professor")) 4.0 else 2.0
  }
  
  # Institutional multiplier (Tier 1 amplifies output via superior infrastructure)
  if (Feature_Tier[i] == "Tier_1") eff <- eff * 1.2
  if (Feature_Tier[i] == "Tier_3") eff <- eff * 0.8
  true_effect[i] <- eff
}

# Final Observation Outcome with Residual Academic Noise
Metric_Y <- baseline_Y + (Action_D * true_effect) + rnorm(N_samples, mean = 0, sd = 3)
Metric_Y <- pmax(0, round(Metric_Y, 2))
Feature_PriorImpact <- round(Feature_PriorImpact, 2)

# Bind everything into the primary downstream dataframe container
raw_data <- data.frame(
  Metric_Y, 
  Action_D, 
  Feature_Rank = as.factor(Feature_Rank), 
  Feature_Field = as.factor(Feature_Field), 
  Feature_Tier = as.factor(Feature_Tier), 
  Feature_YearsPhD, 
  Feature_PriorImpact
)

cat(sprintf("Cohort Synthesis Complete: %d observations loaded into active pipeline.\n", nrow(raw_data)))
