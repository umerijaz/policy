#Policy Objective: Subsidising agricultural smart sensors to optimise water usage against climate change.
...
program_cost    <- 2000    # Cost of smart sensor kit
budget_capacity <- 0.25    # 25% of farms receive subsidies

outcome_var   <- "Metric_Y"
treatment_var <- "Action_D"
covariates    <- c("Feature_SoilQuality", "Feature_CropType", "Feature_FarmSize", "Feature_WaterStress")

N_samples <- 1500
# 1. Farm Characteristics
Feature_SoilQuality <- runif(N_samples, 0, 100)
Feature_CropType <- sample(c("Corn", "Soy", "Wheat"), N_samples, replace = TRUE)
Feature_FarmSize <- runif(N_samples, 10, 500)
Feature_WaterStress <- runif(N_samples, 0, 1)

# 2. Treatment Assignment (Bias: Larger, more tech-savvy farms apply more)
propensity <- 1 / (1 + exp(-(-2 + 0.01 * Feature_FarmSize + 0.5 * Feature_WaterStress)))
Action_D <- rbinom(N_samples, 1, propensity)

# 3. Outcome (Yield in kg/hectare)
true_effect <- 50 + (100 * Feature_WaterStress) + (0.5 * Feature_SoilQuality)
Metric_Y <- 2000 + (10 * Feature_SoilQuality) + (Action_D * true_effect) + rnorm(N_samples, 0, 100)

raw_data <- data.frame(Metric_Y, Action_D, Feature_SoilQuality, Feature_CropType = as.factor(Feature_CropType), Feature_FarmSize, Feature_WaterStress)
cat(sprintf("Irrigation Cohort Synthesis Complete: %d farms loaded.\n", nrow(raw_data)))
