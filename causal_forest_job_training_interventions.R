# Policy Objective: A baseline synthetic template used for generalised system testing (e.g., job training interventions).
...
program_cost    <- 3500    
budget_capacity <- 0.20    # Strict resource boundary (e.g., 25% max capacity)

# Target Key Mapping: Adjust these to match your dataset's column headers exactly
outcome_var   <- "Metric_Y"       
treatment_var <- "Action_D"       
covariates    <- c("Feature_Age", "Feature_Score", "Feature_Geography", "Feature_Tier") 

# Ingestion Stream Configuration
# -> Set this string to your local or network path (e.g., "data/applicant_pool_2026.csv")
# -> Set to NULL to automatically deploy the self-validating active-lift template
csv_path <- NULL 

# Execute Agnostic Ingestion Routine
if (!is.null(csv_path)) {
  if (!file.exists(csv_path)) {
    stop(sprintf("CRITICAL FILE IO ERROR: Target data stream path not found at [%s]. Verify directory routing.", csv_path))
  }
  cat(sprintf("📡 Ingesting production dataset from external stream: [%s]...\n", csv_path))
  raw_data <- read.csv(csv_path, stringsAsFactors = FALSE, check.names = TRUE)
  
} else {
  cat("⚠️ No production CSV detected. Deploying active-lift synthetic template for local validation...\n")
  N_samples         <- 1500
  Feature_Age       <- sample(18:72, N_samples, replace=TRUE)
  Feature_Score     <- runif(N_samples, 5, 95)
  Feature_Geography <- sample(c("Urban", "Rural", "Suburban"), N_samples, replace=TRUE)
  Feature_Tier      <- sample(c("Tier_1", "Tier_2", "Tier_3"), N_samples, replace=TRUE)
  Action_D          <- rbinom(N_samples, 1, 0.45)
  
  # Inject high-potency heterogeneity (Young + Urban group responds aggressively)
  baseline_Y            <- 22000 + (10 * Feature_Score) - (5 * Feature_Age)
  true_treatment_effect <- ifelse(Feature_Age < 35 & Feature_Geography == "Urban", 6500, 500)
  Metric_Y              <- round(baseline_Y + (Action_D * true_treatment_effect) + rnorm(N_samples, sd=1000))
  
  raw_data <- data.frame(Metric_Y, Action_D, Feature_Age, Feature_Score, Feature_Geography, Feature_Tier)
}
