# Policy Objective: Subsidising annual transit passes to reduce private vehicle commuting.
...
program_cost    <- 600     # Cost of annual pass subsidy
budget_capacity <- 0.35    # 35% of eligible commuters receive the subsidy

outcome_var   <- "Metric_Y"
treatment_var <- "Action_D"
covariates    <- c("Feature_IncomeBracket", "Feature_CommuteDistance", "Feature_CurrentMode", "Feature_Age")

N_samples <- 2500
Feature_IncomeBracket <- sample(c("Low", "Mid", "High"), N_samples, replace = TRUE)
Feature_CommuteDistance <- runif(N_samples, 2, 30)
Feature_CurrentMode <- sample(c("Car", "Bicycle", "Walking"), N_samples, replace = TRUE)
Feature_Age <- round(runif(N_samples, 18, 70))

propensity <- 1 / (1 + exp(-(-1 + 0.8 * (Feature_IncomeBracket == "Low") - 0.03 * Feature_CommuteDistance)))
Action_D <- rbinom(N_samples, 1, propensity)

true_effect <- 10 + (2 * Feature_CommuteDistance) + ifelse(Feature_IncomeBracket == "Low", 5, 0)
Metric_Y <- 50 - (Action_D * true_effect) + rnorm(N_samples, 0, 5) # Y is CO2 emissions

raw_data <- data.frame(Metric_Y, Action_D, Feature_IncomeBracket = as.factor(Feature_IncomeBracket), 
                       Feature_CommuteDistance, Feature_CurrentMode = as.factor(Feature_CurrentMode), Feature_Age)
cat("Transit Cohort Loaded.\n")
