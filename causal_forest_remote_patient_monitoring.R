#Policy Objective: Providing Remote Patient Monitoring (RPM) equipment to manage chronic diseases and reduce hospital readmissions.
...
program_cost    <- 1200    # Cost of the RPM equipment and monitoring service
budget_capacity <- 0.20    # Only 20% of high-risk patients can be enrolled

outcome_var   <- "Metric_Y"
treatment_var <- "Action_D"
covariates    <- c("Feature_Age", "Feature_ComorbidityCount", "Feature_DigitalLiteracy", "Feature_DistanceToClinic")

N_samples <- 2500
# 1. Patient Characteristics
Feature_Age <- round(rnorm(N_samples, 65, 10))
Feature_ComorbidityCount <- rpois(N_samples, 2)
Feature_DigitalLiteracy <- sample(c("Low", "Medium", "High"), N_samples, replace = TRUE)
Feature_DistanceToClinic <- runif(N_samples, 5, 100) # Miles

# 2. Treatment Assignment (Selection bias: closer patients get it easily, high risk get it via triage)
propensity <- 1 / (1 + exp(-(-1.5 + 0.1 * Feature_ComorbidityCount - 0.02 * Feature_DistanceToClinic)))
Action_D <- rbinom(N_samples, 1, propensity)

# 3. Outcome (Reduction in hospital readmission costs)
baseline_Y <- 5000 + (1000 * Feature_ComorbidityCount) + (50 * Feature_Age)
true_effect <- 1500 + (500 * Feature_ComorbidityCount) + ifelse(Feature_DigitalLiteracy == "Low", 1000, 0)
Metric_Y <- baseline_Y - (Action_D * true_effect) + rnorm(N_samples, 0, 500)
Metric_Y <- pmax(0, Metric_Y)

raw_data <- data.frame(Metric_Y, Action_D, Feature_Age, Feature_ComorbidityCount, Feature_DigitalLiteracy = as.factor(Feature_DigitalLiteracy), Feature_DistanceToClinic)
cat(sprintf("Telemedicine Cohort Synthesis Complete: %d patients loaded.\n", nrow(raw_data)))
