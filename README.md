...
This repository is for a new framework (`causal_forest_simulations.pdf`) for optimising policy interventions using Generalised Random Forest (GRF) and Policy Tree methodologies. As we navigate current budgetary constraints and to address the need for more efficient resource allocation. These tools can be utilised to transition from reactive implementation to predictive "Active-Lift" strategies.
## Why This Matters
