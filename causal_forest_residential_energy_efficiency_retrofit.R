#Policy Objective: Municipal grants for home insulation and heat pump installations to lower carbon footprints.
...
program_cost    <- 5000    # Grant amount per household
budget_capacity <- 0.15    # Only 15% of homes can be retrofitted annually

outcome_var   <- "Metric_Y"
treatment_var <- "Action_D"
covariates    <- c("Feature_HouseAge", "Feature_HeatingType", "Feature_InsulationLevel", "Feature_EnergyUsage")

N_samples <- 2000
Feature_HouseAge <- round(runif(N_samples, 5, 120))
Feature_HeatingType <- sample(c("Electric", "Gas", "Oil"), N_samples, replace = TRUE)
Feature_InsulationLevel <- sample(c("Poor", "Average", "Good"), N_samples, replace = TRUE)
Feature_EnergyUsage <- rnorm(N_samples, 15000, 3000)

propensity <- 1 / (1 + exp(-(-2 + 0.05 * (120 - Feature_HouseAge))))
Action_D <- rbinom(N_samples, 1, propensity)

true_effect <- 1000 + (0.2 * Feature_EnergyUsage) + ifelse(Feature_HeatingType == "Electric", 500, 0)
Metric_Y <- 2000 - (Action_D * true_effect) + rnorm(N_samples, 0, 200) # Y is energy bill

raw_data <- data.frame(Metric_Y, Action_D, Feature_HouseAge, Feature_HeatingType = as.factor(Feature_HeatingType), 
                       Feature_InsulationLevel = as.factor(Feature_InsulationLevel), Feature_EnergyUsage)
cat("Energy Efficiency Cohort Loaded.\n")
