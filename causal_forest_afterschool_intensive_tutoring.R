#Policy Objective: Targeted after-school mathematics tutoring designed to improve standardised test scores.
...
program_cost    <- 800     # Cost per student for the semester
budget_capacity <- 0.30    # 30% of the student body can be tutored

outcome_var   <- "Metric_Y"
treatment_var <- "Action_D"
covariates    <- c("Feature_GradeLevel", "Feature_SES", "Feature_PriorScore", "Feature_Attendance")

N_samples <- 2000
# 1. Student Characteristics
Feature_GradeLevel <- sample(6:12, N_samples, replace = TRUE)
Feature_SES <- sample(c("Low", "Mid", "High"), N_samples, replace = TRUE, prob = c(0.4, 0.4, 0.2))
Feature_PriorScore <- rnorm(N_samples, 50, 15)
Feature_Attendance <- runif(N_samples, 0.6, 1.0)

# 2. Treatment Assignment (Bias: Low SES/Low Score students targeted by counselors)
propensity <- 1 / (1 + exp(-(-0.5 + 2 * (Feature_SES == "Low") - 0.05 * Feature_PriorScore)))
Action_D <- rbinom(N_samples, 1, propensity)

# 3. Outcome (Test score gain)
true_effect <- 10 + (0.2 * Feature_PriorScore) + ifelse(Feature_SES == "Low", 5, 0)
Metric_Y <- 40 + (0.8 * Feature_PriorScore) + (Action_D * true_effect) + rnorm(N_samples, 0, 5)
Metric_Y <- pmin(100, Metric_Y)

raw_data <- data.frame(Metric_Y, Action_D, Feature_GradeLevel, Feature_SES = as.factor(Feature_SES), Feature_PriorScore, Feature_Attendance)
cat(sprintf("Tutoring Cohort Synthesis Complete: %d students loaded.\n", nrow(raw_data)))
