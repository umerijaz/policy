# Policy Objective: Intensive municipal forestry maintenance to combat the urban heat island effect.
...
program_cost    <- 25000   # Cost per neighborhood maintenance zone
budget_capacity <- 0.20    # 20% of zones are selected for intensive care

outcome_var   <- "Metric_Y"
treatment_var <- "Action_D"
covariates    <- c("Feature_PopulationDensity", "Feature_CanopyPercentage", "Feature_VulnerabilityIndex", "Feature_ZoneSize")

N_samples <- 1200
Feature_PopulationDensity <- runif(N_samples, 1000, 20000)
Feature_CanopyPercentage <- runif(N_samples, 5, 60)
Feature_VulnerabilityIndex <- sample(c("Low", "Medium", "High"), N_samples, replace = TRUE)
Feature_ZoneSize <- runif(N_samples, 50, 500)

propensity <- 1 / (1 + exp(-(-2 + 0.5 * (Feature_VulnerabilityIndex == "High"))))
Action_D <- rbinom(N_samples, 1, propensity)

true_effect <- 2 + (0.05 * Feature_PopulationDensity / 1000) + ifelse(Feature_VulnerabilityIndex == "High", 3, 0)
Metric_Y <- 30 - (Action_D * true_effect) + rnorm(N_samples, 0, 0.5) # Y is avg neighborhood temp

raw_data <- data.frame(Metric_Y, Action_D, Feature_PopulationDensity, Feature_CanopyPercentage, 
                       Feature_VulnerabilityIndex = as.factor(Feature_VulnerabilityIndex), Feature_ZoneSize)
cat("Urban Canopy Cohort Loaded.\n")
